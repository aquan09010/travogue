...
import {
  addMainImage,
  addOtherImageHook,
  createExperienceHook,
  getInsurances,
} from "@/Hooks/HostManageHook";
...
export default function EditTicketPrice({ route }) {
  ...
  const {
    insurances,
    isGetInsurancesLoading,
    getInsurancesError,
    refetchGetInsurances,
  } = getInsurances(accessToken);

  const basicInfo = route.params;

  const { createExperience, newExperience, isCreateExperienceLoading } =
    createExperienceHook();
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.statusBar}>
        ...
        <Text style={styles.title}>Chỉnh sửa Thời gian và Giá vé</Text>
      ...
      >
        ...
        {isCreateExperienceLoading ||
        isAddImageLoading ||
        isAddOtherImageLoading ? (
          <ActivityIndicator
            size="large"
            color="#ED2939"
              ...
              >
                <Text
                  style={[
                    styles.optionText,
                    {
                      color: selectedInsurances.includes(insurance.id)
                        ? "red"
                        : "gray",
                    },
                  ]}
