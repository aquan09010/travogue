...
export default function HostProfileScreen({ route }) {
  ...
  const { host, isHostLoading, error, refetchHostInfo } = getHostInfo(
    accessToken,
    hostId
  );
      ...
      {isHostLoading ? (
        ...
        <ScrollView style={{}}>
          <View style={styles.containerCard}>
            <View>
              ...
              <Pressable
                ...
                onPress={() =>
                  navigation.navigate("ProfileScreen", { userId: host.id })
                }
              >
            ...
            <View style={styles.orderCard}>
              ...
              <Text style={{ paddingTop: 5 }}>
                Tổ chức trải nghiệm trên Travogue từ{" "}
                {host.createdAt.substring(0, 4)}
              </Text>
              <View style={[styles.line, { alignItems: "center" }]}>
                <SvgXml xml={LanguageBlackIcon} />
                <Text style={[styles.text]}> {host.languages}</Text>
              </View>
              <View style={[styles.line]}>
                <SvgXml xml={ListBlackIcon} />
                <Text style={[styles.text]}> {host.personalSkills}</Text>
              </View>
              {/* <View style={[styles.line]}>
          ...
          <View style={styles.mainView}>
            ...
            <Text style={{ paddingBottom: 15 }}>{host.selfIntroduction}</Text>
            {/* <FlatList
            data={DATA}
